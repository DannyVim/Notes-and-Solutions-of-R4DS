...
可是从结果来看，有327150行不相等。可能是因为记录的时间，跨过了午夜，因为数据里有负值；以及这些飞机的航线跨越了时区，按地方时记录的。虽然`airports`里记录了time zone信息，但我不确定用`tz`还是`tzone`。
...
结果有1236个值不同，而且这个差值均为1440分钟。实际上是因为过了零点，而产生的计算上的差误。`time_diff = (dep_delay - delay)%%1440`把这个影响取消掉就可以了。
